...
RemoBFStr_t RemoBF_str = {.DataDepth = DBUFFER_DEPTH * 4,
                          .VariDepth = VBUFFER_DEPTH};
uint32_t RemoBF_DataList_p[DBUFFER_DEPTH] = {0}; /*Define Data Temp Array*/
RemoVari_t RemoBF_VariList_p[VBUFFER_DEPTH] = {
    0};               /*Define variable Temp Array*/
...
int main() {
  ...
  /*Setting up Timer2*/
  HardWareSet_lay(&Pwm2HWSet_str, Pwm2FlagPara_p, Pwm2RegPara_p,
...
ISR(TIMER2_OVF_vect) {
  ...
  cli();
  if (P_D != 0x00000000 && P_D != 0xFFFFFFFF)
    printf("Protected  P_C ERROR  \n");
  sei();
}
