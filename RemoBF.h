 ...
 * @brief Provide memory r/w protection for the variable that booked as remo-IO register
...
/**
 * @brief structure type provides field for whole information about remo R/W variable 
...
/**
 * @brief Layout funtion for the remo R/W buffer connect its structure to its data_list and vari_list
 *
 * @ingroup remobf_func
 ...
 * @return   int16_t: 
 *  0:  OK  
 * -1:  DATA_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -2:  VARI_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -5:  REMOBF_ILLEGAL_PUT_ERROR (Buffer not EMPTY yet)
 * 
 */
int16_t RemoBF_put(RemoBFStr_t* Str_p, uint8_t RegId, uint8_t Data)

/**
 * @brief Copy the contain in Remo buffer to the variable with RegId or  * distributes to all of the variables.  
 *
 * @ingroup remobf_func
 * @param Str_p :pointer to Remo R/W Buffer structure
 * @param RegId :RegId=1~255: the Register Id,  RegId=0:for all the registed variables 
 * @return int16_t
 * >=0 : Residual space in buffer for more recording
 * -1:  DATA_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -2:  VARI_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -6:  REMOBF_ILLEGAL_DIST_ERROR (Buffer not FULL yet)
 */
int16_t RemoBF_dist(RemoBFStr_t* Str_p, uint8_t RegId) 

/**
 * @brief Collect the whole contain in the variable with RegId or all the  * variables and store into Remo Buffer. . 
 *
 * @ingroup remobf_func
 * @param Str_p :pointer to Remo R/W Buffer structure
 * @param RegId :RegId=1~255: the Register Id,  RegId=0:for all the registed variables 
 ...
 * -1:  DATA_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -2:  VARI_BUFFER_NON_EXIST_ERROR (Forget to call RemoBF_lay() before call this function)
 * -7:  REMOBF_ILLEGAL_CATCH_ERROR (Buffer not Empty)
 */
